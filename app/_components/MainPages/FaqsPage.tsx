...
export default function FaqsPage({ data }: Props) {
    ...
    const types = Array.from(
        new Set(
            data.FaqsData
                .map((faq) => faq.attributes.faqs_type?.data?.attributes?.Title?.trim())
                .filter(Boolean)
        )
    ) as string[]
    ...
    const filteredFaqs = activeType
        ? data.FaqsData.filter(
            (faq) => faq.attributes.faqs_type?.data?.attributes?.Title?.trim() === activeType
        )
        : data.FaqsData
            ...
            <div className=" max-w-[1448px] px-4 mx-auto  md:mb-[100px] mb-[50px]">
                <div className="flex flex-wrap gap-4 mb-10">
                    <button
                        ...
                        className={`py-2 px-[14px] rounded-full text-base font-bold transition ${!activeType ? 'bg-primary text-white' : 'text-primary bg-CoolGray'
                            }`}
                    >
                        {t("data.all")}
                    </button>
                    {types.map((type) => (
                        <button
                            ...
                            className={`py-2 px-[14px] rounded-full text-base font-bold transition ${activeType === type ? 'bg-primary text-white' : 'text-primary bg-CoolGray'
                                }`}
                        >
                            {type}
                        </button>
                ...
                <div className='flex justify-between gap-10 items-start lg:flex-row flex-col'>
                    <div className="xl:w-[calc(100%-600px)] lg:w-[calc(100%-450px)] w-full space-y-6">
                        {filteredFaqs.map((faq, index) => (
                            <SingleAccordion
                                item={{ Title: faq.attributes.Question, Description: faq.attributes.Answer }}
                                index={index} key={index}
                                handleClick={handleClick} openFaq={openFaq} Big />
                        ))}
                    </div>
                    ...
                    <div className="xl:w-[500px] lg:w-[400px] w-full bg-beige xl:p-10 p-7 flex flex-col gap-5">
                        ...
                        <p className="text-primary md:text-xl text-base">{data.MainData.Text.Description}</p>
                    </div>
                </div>
